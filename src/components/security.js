...
async function fetchSecurityPin(storedUserId) {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            const securityPin = walletInfo.wallet_information.withdrawal_password;
...
async function updateSecurityPin(userId, newPin) {
    ...
    try {
        await updateDoc(userDocRef, {
            'wallet_information.withdrawal_password': newPin
        });
...
export async function renderSecurityPassword() {
    ...
    element.innerHTML = `
        ...
        <div class="security-pin">
            <label for="oldsecurity-pin" class="pre p-b-3">Old Security Password</label>
            <input type="password" id="oldsecurity-pin" class="withdraw-input" placeholder="Type Security PIN" required>
        </div>
        <div class="security-pin">
            <label for="newsecurity-pin" class="p-b-3 pre">New Security Password</label>
            <input type="password" id="newsecurity-pin" class="withdraw-input" placeholder="Type Security PIN" required>
        </div>
        <div class="security-pin">
            <label for="confirmsecurity-pin" class="pre p-b-3">Confirm New Security Password</label>
    ...
    submitButton.addEventListener('click', async () => {
        ...
        displayError('Security PIN successfully updated', false);
